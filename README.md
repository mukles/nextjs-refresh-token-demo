# Refresh Tokens — Next.js + NestJS Turborepo
...
# 1. Start the project-local MongoDB replica set on port 27018
npm run db:up

# 2. Configure the frontend's public NestJS URL
cp apps/web/.env.example apps/web/.env.local

# 3. Sync the backend-owned schema and seed the demo user
npm run db:push
npm run db:seed
...
apps/
  api/                         NestJS backend (port 3001)
    src/auth/                  login, OTP, refresh, session, and logout APIs
    src/students/              protected student profile API
    src/prisma.service.ts      backend-owned MongoDB access
  ...
  web/                         Next.js frontend (port 3000)
    ...
    lib/
      backend-client.ts          credentialed NestJS client
    proxy.ts                    Next.js 16 request proxy
turbo.json                      shared task graph and build outputs
```

Use `npm run dev`, `npm run build`, `npm run lint`, or `npm run typecheck` at the
repository root to run the matching task across both workspaces. To target one app:
...
Turborepo · NestJS 11 · Next.js 16 (App Router) · TypeScript · [Prisma](https://www.prisma.io) + MongoDB ·
