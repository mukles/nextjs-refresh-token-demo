...
async function internalFetchWithAutoRefresh(
  ...
  const refreshRes = await internalFetch("/api/auth/refresh", {
    method: "POST",
  });
...
export function DashboardClient() {
  ...
  const callProtected = useCallback(async () => {
    ...
    try {
      const { res, didRefresh } =
        await internalFetchWithAutoRefresh("/api/auth/me");
  ...
  const forceRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      const res = await internalFetch("/api/auth/refresh", { method: "POST" });
      if (!res.ok) return handleAuthFailure(res);
  ...
  const logout = useCallback(async () => {
    await internalFetch("/api/auth/logout", { method: "POST" });
